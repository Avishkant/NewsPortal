        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <nav className="hidden md:flex items-center gap-3 ml-6 font-medium">
              ...
              <NavItem to="/contact" className="text-slate-200 hover:text-white">
                Contact
              </NavItem>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                <div className="text-sm font-semibold text-white hidden lg:block">{user.name}</div>
                <NavItem to={dashboardLink} className="bg-white/10 hover:bg-white/20">
            ...
            {open && (
                ...
                >
                    <div className="px-4 py-3 flex flex-col gap-2 text-white text-base font-medium">
                        <Link to="/news" className="px-2 py-2 rounded hover:bg-white/10" onClick={() => setOpen(false)}>All News</Link>
                        <Link to="/about" className="px-2 py-2 rounded hover:bg-white/10" onClick={() => setOpen(false)}>About</Link>
                        <Link to="/contact" className="px-2 py-2 rounded hover:bg-white/10" onClick={() => setOpen(false)}>Contact</Link>
                        
                        {user && (
                            <>
                                <Link to={dashboardLink} className="px-2 py-2 rounded hover:bg-white/10" onClick={() => setOpen(false)}>Dashboard</Link>
                                <button
                        ...
                        {!user && (
                            <div className="flex flex-col gap-2 mt-2 pt-2 border-t border-gray-700">
                                <Link to="/login" className="px-2 py-2 rounded font-semibold text-center" style={{ backgroundColor: SECONDARY_TEAL }} onClick={() => setOpen(false)}>Sign in</Link>
                                <Link to="/signup" className="px-2 py-2 rounded border border-white/20 hover:bg-white/10 text-center" onClick={() => setOpen(false)}>Create account</Link>
                            </div>
                        )}
                    </div>
                </motion.div>
