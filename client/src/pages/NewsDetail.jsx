...
export default function NewsDetail() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/news/${id}`)
      .then((d) => setItem(d))
      .catch(() => showToast({ type: "error", message: "Failed to load article." }))
  ...
  // --- Render Status ---
  if (loading) return <div className="p-10 text-center text-gray-600">Loading article...</div>;
  if (!item) return <div className="p-10 text-center text-red-600">Article not found.</div>;

  const canEdit = user && (user.role === "owner" || item.author?._id === user.id);
      ...
      >
        ...
        <header className="mb-6 pb-4 border-b border-gray-200">
          ...
          <div className="text-sm text-gray-500 flex items-center flex-wrap gap-x-4">
            ...
            <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${isApproved ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
        ...
        <footer className="pt-4 border-t border-gray-200">
            ...
            <div className="text-sm text-gray-600 mb-4 flex items-center justify-between">
                ...
                {item.link && (
                    <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline flex items-center gap-1">
            ...
            <div className="flex flex-wrap items-center gap-4 py-2">
                ...
                <motion.button
                    onClick={() => {
                        ...
                        const text = encodeURIComponent(`${item.title} - Read more: ${url}`);
                ...
                <motion.button
                    onClick={() => {
                        const url = encodeURIComponent(`${window.location.origin}/news/${item._id}`);
                    ...
                    onClick={async () => {
                        ...
                        try {
                            if (navigator.share) {
                                ...
                                showToast && showToast({ type: "success", message: "Link copied to clipboard" });
                            }
                        } catch {
